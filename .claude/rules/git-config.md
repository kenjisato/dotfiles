...
lazygit takes two steps on Windows, because its config home there is `%LOCALAPPDATA%\lazygit` rather than `~/.config`. `etc/deploy.ps1` links `config.yml` under `~/.config/lazygit/` like the bash deploy, and the deployed PowerShell profile exports `LG_CONFIG_FILE` pointing at it — the same guards as `~/.shell/lazygit.sh`, appending `config.local.yml` when it exists, since a listed file that does not exist aborts lazygit. That covers PowerShell sessions only; a lazygit started from cmd, Git Bash or a shortcut reads `%LOCALAPPDATA%` and its own defaults. Promoting the variable to a user-level environment variable would fix that, at the cost of writing machine state the deploy currently does not touch.
